...
candle_count = int(60/time_unit) * 200
...
ma_duration_long = 45   # 장기 ma 산출 기간
...
buy_cri_vol_times = 0.01   # 전 구간 평균 거래량 대비, 직전 구간 (또는 직전직전 구간)에서 거래량이 얼마 이상이여야 buy_transaction을 수행할 것인가
vol_duration = 12   # 몇 개월 동안의 평균 거래량 기준으로 ref vol을 설정할 것인가
ratio_ema_o_short_long_buy_1 = [1.0, 1.05]
ratio_ema_long_1 = 1.0001   # 현재 구간에서의 ema_long값과 직전구간 ema_long값의 비율이 얼마 이상일때 buy transaction을 수행하는가 (여러 필요 조건중의 하나)
...
sell_normal_profit_ratio = 0.075   # Normal Selling 판매시 이익율
sellable_profit = 0.0   # 판매가능 이익율
sell_force_loss = 0.1   # 강제 판매 손실율
sell_ma_long_ratio = 1.00079   # 1.00059
slow_sto_k_cri = 0.9
slow_sto_k_count = 1
...
if DF_volume_cri['open'][0] >= 1000000 :  # 200만원 이상은 거래단위가 1000원, 100~200만원은 거래단위가 500원이지만 편의상 200만원 이상과 함께 처리
    unit_factor = -3
    unit_value = 1000
elif DF_volume_cri['open'][0] >= 100000 :
    unit_factor = -2
    unit_value = 50
elif DF_volume_cri['open'][0] >= 10000 :
    unit_factor = -1
    unit_value = 10
elif DF_volume_cri['open'][0] >= 1000 :
    unit_factor = -1
    unit_value = 5
elif DF_volume_cri['open'][0] >= 100 :
    unit_factor = 0
    unit_value = 1
else :
    DF_volume_cri['open'][0] <= 100   # 100원 미만은 별도로 code에서 int형이 아닌 float형으로 형변환 해줘야함
    unit_factor = 1
    unit_value = 0.1

print ('DF_volume_cri[open][0] : {0}  / unit_value : {1}'.format(DF_volume_cri['open'][0], unit_value))
...
while True:
    ...
    if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
        ...
        if bought_state == 0 :   # 매수가 없는 상태라면
            ...
            '''
            if ((DF_test_stochastic['ema_o_l_consecutive_rise'][-(ema_long_duration + 1) : -1].sum() > 5) and \
            (DF_test_stochastic['ema_o_s_consecutive_rise'][-(ma_short_under_duration + 1) : -1].sum() > (ma_short_under_duration - 2)) and \
            ((DF_test_stochastic['ema_open_long'][-1] / DF_test_stochastic['ema_open_long'][-2]) > ratio_ema_long_1) and \
            ((DF_test_stochastic['ratio_vol_to_aver'][-3] > buy_cri_vol_times) or (DF_test_stochastic['ratio_vol_to_aver'][-2] > buy_cri_vol_times)) and \
            (DF_test_stochastic['ema_open_short'][-1] / DF_test_stochastic['ema_open_long'][-1] > ratio_ema_o_short_long_buy_1[0]) and (DF_test_stochastic['ema_open_short'][-1] / DF_test_stochastic['ema_open_long'][-1] < ratio_ema_o_short_long_buy_1[1])) or \
            ((DF_test_stochastic['ema_o_l_consecutive_rise'][-(ema_long_duration + 1) : -1].sum() > 6) and \
             (DF_test_stochastic['ema_o_s_consecutive_rise'][-(ma_short_under_duration + 1) : -1].sum() > 0) and \
             ((DF_test_stochastic.loc[DF_test_stochastic.iloc[-(decrese_day_duration + 1) : -1]['ema_open_long'].idxmin()]['ema_open_long'] * 1.0019) > DF_test_stochastic['ema_open_long'][-1]) and \
             ((DF_test_stochastic['ratio_vol_to_aver'][-3] > (0.7 * buy_cri_vol_times)) or (DF_test_stochastic['ratio_vol_to_aver'][-2] > (0.7 * buy_cri_vol_times))) and \
             (DF_test_stochastic['ema_open_short'][-1] / DF_test_stochastic['ema_open_long'][-1] > ratio_ema_o_short_long_buy_2[0]) and (DF_test_stochastic['ema_open_short'][-1] / DF_test_stochastic['ema_open_long'][-1] < ratio_ema_o_short_long_buy_2[1]) and \
             ((DF_test_stochastic['ema_open_long'][-1] / DF_test_stochastic['ema_open_long'][-2]) > ratio_ema_long_2)) :
            '''
